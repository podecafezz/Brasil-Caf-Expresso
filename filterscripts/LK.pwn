...
//---------------------------------------- Hotel -----------------------------------------------
 	SetPlayerMapIcon(playerid, 0, 328.5529, -1513.7094, 36.0391, 47, 0); // LS
    //---------------------------------------- DriveThru --------------------------------------------
...
{
	if(!IsPlayerInAnyVehicle( playerid )) return SendClientMessage( playerid, VERMELHO, "| Drive Thru | Você não está em um veículo !" );
...
CMD:setsono(playerid)
{
	...
	SendClientMessage(playerid, -1, "[!] Sua barra de sono foi setada para 5");
	return 1;
}
...
CMD:rsono(playerid)
{
	...
	SendClientMessage(playerid, -1, "[!] Sua barra de sono foi recuperada");
	return 1;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == D_HOTEL)
	{
		if(response)
		{
            ...
            if(GetPVarInt(playerid,"SONO") >= 75) return SendClientMessage(playerid,VERMELHO,"[!] Você não esta com sono!");
            SendClientMessage(playerid, -1, "| INFO | Use '/Dormir' em cima da cama para poder dormir !");
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if(checknome == CheckC_HT)
	{
		...
		SendClientMessage(playerid, 0xFFFFFFFF, " /AlugarQuarto - Para alugar um quarto no valor de $100");
		SendClientMessage(playerid, 0x008282AA, "================ Hotel =================");
	}
...
public SEDE(playerid)
{
	...
	if(GetPVarInt(playerid,"SEDE") <= 5)
	{
		...
		SendClientMessage(playerid, 0xFF0000FF,"[!] Vá até a lanchonete comer ou irá morrer de Sede!");
	}
...
public SONO(playerid)
{
	...
	{
		SendClientMessage(playerid,-1,"[!] Você esta passando mal por excesso de sono, se não durmir podera desmaiar.");
	...
	if(GetPVarInt(playerid,"SONO") <= 0)
	{
		...
		ApplyAnimation(playerid,"CRACK","crckdeth4",4.0,0,0,0,1,0);
		KillTimer(ContagemSono[playerid]);
    	SendClientMessage(playerid,0xDCDCDCFF,"[!] Você tirou uma soneca por excesso de sono...");
