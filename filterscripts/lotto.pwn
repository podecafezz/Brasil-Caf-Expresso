...
dcmd_lottoprice(playerid, params[])
{
	...
	if(!strlen(params)) return SendClientMessage(playerid,COLOR_WHITE,"Use: /lottoprice [preço]");
...
{
	if(!strlen(params)) return SendClientMessage(playerid,COLOR_WHITE,"Use: /winlotto [id]");
	new endid = strval(params);
	if(!IsPlayerConnected(endid)) return SendClientMessage(playerid,RED,"Jogador Invalido!");
	if(!IsPlayerAdmin(playerid)) return false;
	new name[MAX_PLAYER_NAME];
	GetPlayerName(endid, name, MAX_PLAYER_NAME);
	format(string,256,"Você forço %s (ID: %d) a ganhar a proxima loteria!",name,endid);
...
dcmd_lottotime(playerid, params[])
{
	...
	if(!strlen(params)) return SendClientMessage(playerid,COLOR_WHITE,"Use: /lottotime [segundos]");
